...
export function UsuariosList() {
  ...
  const handleDelete = (id: string) => {
    toast.warn(
      ({ closeToast }) => (
        <div>
          <p style={{ margin: '0 0 10px 0', color: '#1e293b', fontSize: '14px', fontWeight: 'bold' }}>
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.id}>
                ...
                <td data-label="Data de Criação">{format(parseISO(user.createdAt), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}</td>
                <td data-label="Ações">
                  <div className="action-buttons">
                    <button className="btn-icon btn-edit" onClick={() => handleEdit(user)} title="Editar" type="button">
                      <span className="material-icons">edit</span>
                    </button>
                    <button className="btn-icon btn-delete" onClick={() => handleDelete(user.id)} title="Excluir" type="button">
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="form-group icon-input-group">
                ...
                <input id="name" type="text" name="name" placeholder="Username" value={formData.name} onChange={handleInputChange} />
              </div>
